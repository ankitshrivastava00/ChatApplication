    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 1) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                //  String countryCd = data.getStringExtra("clCode").replace("+", "").replace(" ", "").trim();
                //   mtvCountryCode.setText("+" + countryCd);
    ...
    private void loginSocket() {
        try {
            if (!cd.isConnectingToInternet()) {
                ...
                pd.show();
                StringRequest stringRequest = new StringRequest(Request.Method.POST, "http://128.199.222.145/reg/reg.php",
